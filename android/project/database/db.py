...
def update_user(data):
  ...
  if KEY.NICKNAME in data:
	if check_user_exist({KEY.ACCOUNT: data[KEY.NICKNAME]}):	
		data[KEY.NICKNAME] = MySQLdb.escape_string(data[KEY.NICKNAME].encode("utf8"))
		sql = "update user set nickname = '%s' where id = %d"
		sql_addition = "update account set account = '%s' where id = %d"
		try:
		  dbhelper.execute(sql%(data[KEY.NICKNAME], data[KEY.ID]))
		  dbhelper.execute(sql_addition%(data[KEY.NICKNAME], data[KEY.ID]))
		  result &= True
		except:
		  result &= False
	else:
...
def get_events(data, get_event_id_list, limit=0):
	...
	for event_id in event_id_list:
		...
		user = {}
		user = get_user_information({KEY.ID: event_info[KEY.LAUNCHER_ID]})
		event_info[KEY.IS_VERIFY] = user[KEY.IS_VERIFY]
		event_info[KEY.REPUTATION] = user[KEY.REPUTATION]
		event_info[KEY.OCCUPATION] = user[KEY.OCCUPATION]
...
def add_support_relation(data):
	...
	'''first we need to check whether the event was canceled or completed'''
	event_info = get_event_information({KEY.EVENT_ID: data[KEY.EVENT_ID]})
	if event_info[KEY.STATE] != 0:
		return -1

	sql = "replace into support_relation (event_id, supportee, supporter, type, time) values (%d, %d, %d, %d, now())"
	try:
		n = dbhelper.execute(sql%(data[KEY.EVENT_ID], data[KEY.LAUNCHER_ID], data[KEY.ID], data[KEY.TYPE]))
		if n > 0:
			return n
	except Exception, e:
		print e
...
@params includes id, user's id which is supporter's id
                            event_id, id represents the event
							launcher_id, supportee's id
							operation indicates the type of operation, -1 for system recommends, in notifying,
							                                                                  0 for refuse help or quit the event by user,
																							  1 and 2 for different type of event that user take part in.
@return True if succeed.
               False if fail.
'''
def user_event_manage(data):
	if KEY.ID not in data or KEY.EVENT_ID not in data or KEY.LAUNCHER_ID not in data:
		return False
	if KEY.OPERATION not in data:
		return False
	if data[KEY.OPERATION] < -1 or data[KEY.OPERATION] > 2:
		return False
	...
	if data[KEY.OPERATION] == 0:
		sql = "delete from support_relation where event_id = %d and supportee = %d and supporter = %d"%(data[KEY.EVENT_ID], data[KEY.LAUNCHER_ID], data[KEY.ID])
	else:
		sql = "replace into support_relation (event_id, supportee, supporter, type, time) values (%d, %d, %d, %d, now())"%(data[KEY.EVENT_ID], data[KEY.LAUNCHER_ID], data[KEY.ID], data[KEY.OPERATION])
...
def remove_static_relation(data):
	...
	try:
		...
		dbhelper.execute(sql%(STATUS.REFUSE, data[KEY.USER_ID], data[KEY.ID]))
		if n > 0:
			return True
		else:
			return False
...
'''get user's static relation list.
@params includes user's id and the relation type(optional)
@return the relation list's id.
'''
...
def get_supporters(data):
	...
	if KEY.EVENT_ID not in data:
		return sup_list
	sql = "select supporter, type from support_relation where event_id = %d"%data[KEY.EVENT_ID]
	if KEY.TYPE in data:
		sql += " and type = %d"%data[KEY.TYPE]
	sql_result = dbhelper.execute_fetchall(sql)
	for each_result in sql_result:
...
'''
  update users' family love coin.
  every time when two person become static relation, then upodate their account
  @params includes: id, user's id 
  @return return true if successfully update
          return false when otherwise
'''
def update_family_love_coin(data):
  if KEY.ID not in data or KEY.USER_ID not in data or KEY.TYPE not in data:
    return False
  else:
    user1_info = get_user_loving_bank( {KEY.USER_ID:data[KEY.ID]} )
    user2_info = get_user_loving_bank( {KEY.USER_ID:data[KEY.USER_ID]} )
    sql="update loving_bank set family_love_coin = %d where userid = %d"
    try:
      if data[KEY.TYPE] == 1:
        dbhelper.execute(sql%( user1_info[KEY.FAMILY_LOVE_COIN]+user2_info[KEY.LOVE_COIN],data[KEY.ID] ))
        dbhelper.execute(sql%( user2_info[KEY.FAMILY_LOVE_COIN]+user1_info[KEY.LOVE_COIN],data[KEY.USER_ID] ))
      else:
        dbhelper.execute(sql%( user1_info[KEY.FAMILY_LOVE_COIN]-user2_info[KEY.LOVE_COIN],data[KEY.ID] ))
        dbhelper.execute(sql%( user2_info[KEY.FAMILY_LOVE_COIN]-user1_info[KEY.LOVE_COIN],data[KEY.USER_ID] ))
      return True
...
def update_loving_bank(data):
  ...
  if data[KEY.OPERATION] == 0:
    ...
    update_family_coin = bank_info[KEY.FAMILY_LOVE_COIN] + data[KEY.LOVE_COIN]
  ...
  sql = "update loving_bank set love_coin = %d, score_rank = %d, family_love_coin = %d where userid = %d"  
  dbhelper.execute(sql%(update_love_coin, update_score, update_family_coin, data[KEY.ID]))
  
  friends = get_static_relation({KEY.ID:data[KEY.ID]})
    
  for each_result in friends:
    newuser = get_user_loving_bank({KEY.USER_ID:each_result[KEY.ID]})
    sql = "update loving_bank set family_love_coin = %d where userid = %d"
    if(data[KEY.OPERATION]==0):
      dbhelper.execute(sql%(newuser[KEY.FAMILY_LOVE_COIN]+data[KEY.LOVE_COIN],each_result[KEY.ID]))
    else:
      dbhelper.execute(sql%(newuser[KEY.FAMILY_LOVE_COIN]-data[KEY.LOVE_COIN],each_result[KEY.ID]))   
...
def love_coin_transfer(data):
  ...
  result = True
  update_sender_coin = {}
  update_sender_coin[KEY.ID] = data[KEY.SENDER]
  update_sender_coin[KEY.LOVE_COIN] = data[KEY.LOVE_COIN]
  update_sender_coin[KEY.OPERATION] = 1
  update_sender_coin[KEY.SCORE] = 0
  if(update_loving_bank(update_sender_coin)):
    result = True
  else:
    return False

  update_receiver_coin = {}
  update_receiver_coin[KEY.ID] = data[KEY.RECEIVER]
  update_receiver_coin[KEY.OPERATION] = 0
  update_receiver_coin[KEY.LOVE_COIN] = data[KEY.LOVE_COIN]
  update_receiver_coin[KEY.SCORE] = 0
  if(update_loving_bank(update_receiver_coin)):
    result = True
  else:
    return False
...
def get_transfer(data):
	...
	try:
		...
		if sql_result is not None:
			...
			if(data[KEY.USER_ID] == sql_result[1]):
				record_info[KEY.NICKNAME] = get_userName( {KEY.ID:sql_result[2]} )[0]
				record_info[KEY.LOVE_COIN] = -sql_result[3]
			else:
				record_info[KEY.NICKNAME] = get_userName( {KEY.ID:sql_result[1]} )[0]
				record_info[KEY.LOVE_COIN] = sql_result[3]
...
def check_transfer(data):
  ...
  sql = "select id from coin_exchange where sender = %d or receiver = %d"%(data[KEY.ID],data[KEY.ID])
  sql += " order by time DESC"
  sql_result = dbhelper.execute_fetchall(sql)
  h_list = []
  for each_result in sql_result:
    record = {}
    record[KEY.ID] = each_result
    
    record[KEY.USER_ID] = data[KEY.ID]
...
def reward(data):
  ...
  sup_event[KEY.TYPE] = event[KEY.TYPE]
  ...
  try:
    update_info = {}
    update_info[KEY.ID] = data[KEY.ID]
    update_info[KEY.OPERATION] = 1
    update_info[KEY.LOVE_COIN] = reward_coin
    update_info[KEY.SCORE] = 0
    if not update_loving_bank(update_info):
      return False      
  except Exception, e:
    return False
  
  #add_sql = "update loving_bank set love_coin = love_coin + %d where userid = %d"
  history_sql = "insert into coin_trade (eventid, sender, receiver, lovecoin, time) values (%d, %d, %d, %d, now())"
  if num == 0:
    return True
    ...
    avg_coin = int(reward_coin / VALUE.reward_factor)
    try:
      for i in range(0, num):
        ...
        update_info = {}
        update_info[KEY.ID] = supporters[i][KEY.ID]
        update_info[KEY.OPERATION] = 0
        update_info[KEY.LOVE_COIN] = reward_coin
        update_info[KEY.SCORE] = 5
        if (update_loving_bank(update_info)):
          continue
        else:
          return False
      return True
...
def get_trade(data):
  if KEY.ID not in data:
    return -1
  sql = "select * from coin_trade where sender = %d or receiver = %d"%(data[KEY.ID],data[KEY.ID])
  sql +=" order by time DESC"
...
'''
delete all event_like about event by event_id
@params includes  event's id
@return True if it success
               False otherwise.

'''

def delete_event_like_by_eventId(data):
  if KEY.EVENT_ID not in data:
    return False
  sql = "update event_like set valid = 0 where event_id = %d"
  try:
    dbhelper.execute(sql%(data[KEY.EVENT_ID]))
    return True
  except Exception, e:
    return False
...
'''
user delete the question event
@params include user’s id, event's id
@return True if succeed,
               False otherwise.
'''
def delete_question_event(params):
  
  if KEY.ID not in params or KEY.EVENT_ID not in params:
    return False
  eventInfo = get_event_information(params)
  if eventInfo[KEY.LAUNCHER_ID] != params[KEY.ID] or eventInfo[KEY.TYPE] != 0:
    return False

  try:
    #delete all the answer about the question
    '''
    这里测试发现  删不删回复无所谓
    因为他们是通过外键相关联的 删除事件id 会自动删除所有回复
    '''
    answerIdList = get_answer_id_list(params)
    for answerId in answerIdList:
      answer_Info = {}
      answer_Info[KEY.ANSWER_ID] = answerId
      answer_Info[KEY.EVENT_ID] = params[KEY.EVENT_ID]
      answer_Info[KEY.VALID] = 0
      update_answer(answer_Info)

    #delete all the like about the question
    doneDeleteLike = delete_event_like_by_eventId(params)
    if doneDeleteLike is False:
      return False
    #delete the question event
    sql = "delete from event where id = %d"
    n = dbhelper.execute(sql%params[KEY.EVENT_ID])
    if n > 0:
      return True
  except Exception, e:
    return False